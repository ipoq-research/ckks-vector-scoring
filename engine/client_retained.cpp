...
struct Client {
    ...
    void fast_decode(double* output,bool half=false,bool detailed=false,int variant=0) {
        double t0=clock_us();
        auto cd=context->get_context_data(plain.parms_id());
        auto &mod=cd->parms().coeff_modulus();size_t limbs=mod.size();
        if(!is_valid_for(plain,*context) || !plain.is_ntt_form() || limbs<1 || limbs>2)
            throw std::invalid_argument("Invalid plaintext for specialized decoder");
        double validated=detailed?clock_us():0;
        std::memcpy(coefficients.data(),plain.data(),N*limbs*sizeof(uint64_t));
        ...
        double t1=clock_us();
        const uint64_t p0=mod[0].value();double inverse_scale=1.0/plain.scale();
        if(limbs==1) {
            for(size_t i=0;i<N;i++) {
...
#ifdef SEAL_USE_INTEL_HEXL
        auto &mod=context->get_context_data(ciphertext.parms_id())->parms().coeff_modulus();
        plain.parms_id()=parms_id_zero;plain.resize(N*mod.size());
        for(size_t j=0;j<mod.size();j++) {
            size_t offset=j*N;auto p=mod[j].value();auto *destination=plain.data()+offset;
    ...
    void run(int mode,double *output) {
        if(mode<0 || mode>8)throw std::invalid_argument("Unknown client mode");
        if(mode>=6)vector_decrypt();
        ...
        } else fast_decode(output,mode>=3,false,mode==8?2:mode==7?1:0);
    }
};
...
int client_decode(void* h,const uint8_t* bytes,size_t count,int mode,double* output,size_t outputs) {
    try{
        ...
        auto &c=*static_cast<Client*>(h);c.load(bytes,count);c.run(mode,output);return 1;
    }catch(const std::exception&e){error_text=e.what();return 0;}
}
...
int client_profile_current(void* h,const uint8_t* bytes,size_t count,int mode,double* output,double* phases) {
    try{
        if(!h || !bytes || !phases || !output || (mode!=4 && mode!=6 && mode!=7 && mode!=8))throw std::invalid_argument("Invalid current-profile arguments");
        auto &c=*static_cast<Client*>(h);
        double a=clock_us();c.load(bytes,count);double b=clock_us();
        if(mode>=6)c.vector_decrypt();else c.preconditioned_decrypt();
        double d=clock_us();c.fast_decode(output,true,true,mode==8?2:mode==7?1:0);
